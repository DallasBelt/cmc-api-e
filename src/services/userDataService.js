...
async function create(req, res) {
  try {
    ...
    if (errors.isEmpty()) {
      const { idNumber, dob, firstName, lastName, phone, address, idTypeId } = req.body;
      
      if (await UserData.findOne({ where: { idNumber }})) {
        return res.status(400).json({ message: 'Error!'});
      }
      
      const idType = await IdType.findOne({ where: { name: idTypeId }, attributes: ['id'] });
      await UserData.create({ idNumber, dob, firstName, lastName, phone, address, idTypeId: idType.dataValues.id, userId: user.id });
